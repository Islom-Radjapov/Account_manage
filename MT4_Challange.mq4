...
void OnTick() {
   ...
   if (AccountEquity() < daily_loss) {
      ...
      for (x = 0; x < ArraySize(chat_id); x++) {
         SendMessage(chat_id[x], "Daily loss done   Time - " + TimeToString(TimeCurrent()) + "   Name - " + AccountInfoString(ACCOUNT_NAME) + "   Login - " + AccountInfoInteger(ACCOUNT_LOGIN));
      }
      Write_loss();
   }
      ...
      for (x = 0; x < ArraySize(chat_id); x++) {
         SendMessage(chat_id[x], "Max Drawdown done   Time - " + TimeToString(TimeCurrent()) + "   Name - " + AccountInfoString(ACCOUNT_NAME) + "   Login - " + AccountInfoInteger(ACCOUNT_LOGIN));
      }
      Write_loss();
   }
      ...
      if (OrderSelect(OrdersHistoryTotal() -1 , SELECT_BY_POS, MODE_HISTORY) ) {
         if (OrderCloseTime() + 60 * 10 < TimeCurrent()) {
            Print("Target done");
            for (x = 0; x < ArraySize(chat_id); x++) {
               SendMessage(chat_id[x], "Target done   Time - " + TimeToString(TimeCurrent()) + "   Name - " + AccountInfoString(ACCOUNT_NAME) + "   Login - " + AccountInfoInteger(ACCOUNT_LOGIN));
            }
            Write_loss();
         }
      }
